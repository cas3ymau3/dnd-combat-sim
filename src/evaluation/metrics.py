...
class DaySample:
    ...
    def mitigation(self, attr: str) -> float:
        """A summed field of the §13 mitigation channel, across damage types."""
        return float(sum(getattr(m, attr) for m in self.telemetry.mitigation.values()))
...
def _requires_resource_telemetry(ctx: RunContext) -> "str | None":
    return (
        "CombatTelemetry.record_resource has no call site in resolution — the economy "
        "channel's resource ledger is scaffolding, not data. Until it is written, a "
        "resources-per-day number would be a fabricated zero."
    )
...
def _build_default_registry() -> MetricRegistry:
    ...
    # -- panel: limited resources — UNAVAILABLE (channel has no call site) -
    reg(MetricDef(
        name="limited_resources_per_day",
        unit="uses/day",
        denominator="days",
        source="§13 economy channel — resources_spent",
        definition=(
            "Limited-resource expenditures per adventuring day (spell slots, "
            "Channel Divinity, superiority dice, …)."
        ),
        numerator=lambda s: float(sum(s.telemetry.resources_spent.values())),
        availability=_requires_resource_telemetry,
    ))
