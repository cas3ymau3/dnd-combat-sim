   ...
   - ~~**HEALING SUBSYSTEM (inserted ahead of step 3; user, s44)**~~ **BUILT (s45)** — see the
     ...
     - **Hit Dice (§7):** the CHARACTER and party spend all HD automatically at the short
       ...
       **SUMMONS instead spend HD after EACH COMBAT to heal their DEFICIT (`max_hp − hp`),
       ...
       so the cap question does not arise. Two rules because they measure two things: the
       character's is POTENTIAL healing (nothing reads a character's `hp`), the summon's is
       ACTUAL healing (its HP gates death). Gives a pool-bounded depletion curve,
